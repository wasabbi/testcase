...
void *transition_func(void *arg){
    ...
    if(__orig_start_routine == thread1){ 
        ...
        wait_manage_end = 0;
    	hw_bp_insert(tid, 0, 0, 4);   //start of __start_routine
    }
    	...
    	hw_bp_insert(tid, 0, 1, 4);   //start of __start_routine
    }
        ...
        while(wait_manage_end == 1) { }
    	hw_bp_insert(tid, 0, 2, 4);   //start of __start_routine
    }
    __orig_start_routine(__orig_arg);
    hw_bp_insert(tid, 0, 0, 5);   //end of __start_routine
    thread_end ++;
